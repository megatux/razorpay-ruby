...
module Razorpay
  ...
  class RazorpayPaymentTest < Minitest::Test
    ...
    def test_payments_should_be_available
      payment = Razorpay::Payment.new(@client).fetch(@payment_id)
    ...
    def test_payments_fetch_downtime
      ...
      payments = Razorpay::Payment.new(@client).fetch_payment_downtime
    ...
    def test_payments_fetch_downtime_by_id
      ...
      payment = Razorpay::Payment.new(@client).fetch_payment_downtime_by_id(@downtime_id)
    ...
    def test_payments_fetch_card_details
      ...
      card = Razorpay::Payment.new(@client).fetch_card_details(@payment_id)
    ...
    def test_payments_fetch_multiple_refund_for_payment
      ...
      payments = Razorpay::Payment.new(@client).fetch_multiple_refund(@payment_id,{})
    ...
    def test_payments_fetch_transfers
      ...
      transfers = Razorpay::Payment.new(@client).fetch(@payment_id).fetch_transfer
    ...
    def test_payments_post_transfers
      ...
      transfer = Razorpay::Payment.new(@client).fetch(@payment_id).transfer(param_attr.to_json)
    ...
    def test_payments_fetch_refunds
      ...
      refund = Razorpay::Payment.new(@client).fetch(@payment_id).fetch_refund(@refund_id)
    ...
    def test_payment_create_recurring
       ...
       payment = Razorpay::Payment.new(@client).create_recurring_payment payment_attr.to_json
       assert_equal 'pay_1Aa00000000001', payment.razorpay_payment_id

    end

    def test_all_payments
      payments = Razorpay::Payment.new(@client).all
      assert_instance_of Razorpay::Collection, payments, 'Payments should be an array'
      assert !payments.items.empty?, 'Payments should be more than one'
    end
    ...
    def test_all_payments_with_options
      ...
      payments = Razorpay::Payment.new(@client).all(query)
    ...
    def test_payment_refund
      ...
      refund = Razorpay::Payment.new(@client).fetch(@payment_id).refund
    ...
    def test_partial_refund
      ...
      refund = Razorpay::Payment.new(@client).fetch(@payment_id).refund(amount: 2000)
      assert_instance_of Razorpay::Refund, refund
      assert_equal refund.payment_id, "pay_FFX5FdEYx8jPwA"
    ...
    def test_payment_capture
      ...
      payment = Razorpay::Payment.new(@client).fetch(@payment_id).capture(@payment_id, para_attr.to_json)
    ...
    def test_otp_generate
      ...
      payment = Razorpay::Payment.new(@client).otp_generate(payment_id)
    ...
    def test_otp_submit
      ...
      payment = Razorpay::Payment.new(@client).fetch(@payment_id).otp_submit(param_attr.to_json)
      assert_equal @payment_id, payment.razorpay_payment_id
    end

    def test_otp_resend
      stub_post(%r{payments/#{@payment_id}/otp/resend$}, 'fake_otp_resend', {})
      payment = Razorpay::Payment.new(@client).fetch(@payment_id).otp_resend
      assert_equal @payment_id, payment.razorpay_payment_id
    end
    ...
    def test_payment_edit
      ...
      payment = Razorpay::Payment.new(@client).fetch(@payment_id).edit(payment_attr.to_json)
    ...
    def test_payment_create_payment_json
      ...
      payment = Razorpay::Payment.new(@client).create_json_payment payment_attr.to_json
    ...
    def test_create_upi
      ...
      payment = Razorpay::Payment.new(@client).create_upi param_attr.to_json
      assert_equal 'pay_FVmAstJWfsD3SO', payment.razorpay_payment_id
    end

    def test_payment_methods
      stub_get(/methods$/, 'payment_methods_collection')
      methods = Razorpay::PaymentMethods.new(@client).all
    ...
    def test_validate_vpa
      ...
      payment = Razorpay::Payment.new(@client).validate_vpa param_attr.to_json
