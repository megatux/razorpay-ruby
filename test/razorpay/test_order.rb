...
module Razorpay
  ...
  class RazorpayOrderTest < Minitest::Test
    ...
    def test_order_should_be_created
      ...
      order = Razorpay::Order.new(@client).create amount: 5000, currency: 'INR', receipt: 'TEST'
    ...
    def test_fetching_all_orders
      ...
      orders = Razorpay::Order.new(@client).all
    ...
    def test_order_payments_should_be_fetched
      ...
      payments = Razorpay::Order.new(@client).fetch(@order_id).payments
   ...
   def test_edit_order
      ...
      order = Razorpay::Order.new(@client).edit(@order_id, param_attr.to_json)
      assert_instance_of Razorpay::Order, order, 'order not an instance of Razorpay::Order class'
      assert_equal @order_id, order.id, 'order IDs do not match'
   end
   ...
   def test_fetch_order_transfers
    ...
    order = Razorpay::Order.new(@client).fetch_transfer_order(@order_id)
    assert_instance_of Razorpay::Order, order, 'order not an instance of Razorpay::Order class'
    assert_equal @order_id, order.id, 'order IDs do not match'
