...
module Razorpay
  # Tests for Razorpay::Item.new(@client)
  class RazorpayItemTest < Minitest::Test
    ...
    def test_item_should_be_created
      ...
      items = Razorpay::Item.new(@client).create(param_attr.to_json)
    ...
    def test_fetch_specific_item
      item = Razorpay::Item.new(@client).fetch(@item_id)
      assert_instance_of Razorpay::Entity, item
      assert_equal @item_id, item.id, 'Item IDs do not match'
    end
    ...
    def test_delete_item
      ...
      item = Razorpay::Item.new(@client).delete(@item_id)
      assert_instance_of Razorpay::Entity, item
    end
  end
end
