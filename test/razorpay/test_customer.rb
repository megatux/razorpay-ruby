...
module Razorpay
  ...
  class RazorpayCustomerTest < Minitest::Test
    ...
    def test_customer_should_be_created
      ...
      customer = Razorpay::Customer.new(@client).create email: 'test@razorpay.com', contact: '9876543210'
    ...
    def test_edit_customer
      ...
      customer = Razorpay::Customer.new(@client).edit(@customer_id, "email=#{new_email}")
    ...
    def test_fetch_all_customers
      ...
      customers = Razorpay::Customer.new(@client).all
      assert_instance_of Razorpay::Collection, customers
    end

    def test_fetch_specific_customer
      stub_get(%r{customers/#{@customer_id}$}, 'fake_customer')
      customer = Razorpay::Customer.new(@client).fetch(@customer_id)
      assert_instance_of Razorpay::Customer, customer
      assert_equal "test@razorpay.com", customer.email
    end
    ...
    #   tokens = Razorpay::Customer.new(@client).fetch(@customer_id).fetchTokens
    ...
    def test_customer_fetch_token
      ...
      token = Razorpay::Customer.new(@client).fetch(@customer_id).fetchToken("token_FHfn3rIiM1Z8nr")
    ...
    def test_customer_delete_token
      ...
      token = Razorpay::Customer.new(@client).fetch(@customer_id).deleteToken("token_FHfn3rIiM1Z8nr")
